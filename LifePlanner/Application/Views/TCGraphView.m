...
- (int)indexOfVertexForTouchLocation:(CGPoint)touchPoint
{
    float minDistance = FLT_MAX;
    int index = 0;

    for (NSInteger i = 0; i < self.graph.vertexCount; i++) {
        float d = distance(touchPoint, self.vertexPoints[i]);
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (NSInteger v = 0; v < self.graph.vertexCount; v++) {
        ...
        CGPoint center = self.vertexPoints[v];
        NSArray *adj = [[self.graph enumeratorForVertex:v] allObjects];

        for (NSNumber *w in adj) {
            CGPoint adjPoint = self.vertexPoints[[w integerValue]];
